...
const (
	...
	defaultTaskCleanupIntervalInMinutes = 90
...
func ClearTaskInfoObjects() {
	...
	ticker := time.NewTicker(time.Duration(defaultTaskCleanupIntervalInMinutes) * time.Second)
	for range ticker.C {
...
func (m *volumeManager) CreateVolume(spec *cnstypes.CnsVolumeCreateSpec) (*cnstypes.CnsVolumeId, error) {
	...
	if ok {
		...
		volumeTaskMap[spec.Name] = createVolumeTaskDetails{taskInfo, time.Now().Add(time.Minute * time.Duration(defaultOpsExpirationTimeInHours))}
	}
